...
public class TaskAdapter extends RecyclerView.Adapter<TaskAdapter.TaskViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TaskViewHolder holder, int position) {
        ...
        holder.status.setChecked(toBoolean(task.getIsComplete()));
        holder.status.setOnCheckedChangeListener((compoundButton, b) -> {
            if(holder.status.isChecked()){
                updateStatusID(task.getTaskId(),1);
            }else {
                updateStatusID(task.getTaskId(),0);
            }
        });
    ...
    private void updateStatusID(int taskId, int i) {
